...
setup_dotfiles() {
	...
	ln -sfn ~/dotfiles/aerc ~/.config/aerc
...
system_update(){
	apt update -y
	apt upgrade -y
	msg_ok "Hackdroid wurde erfolgreich aktualisiert."
}
...
select option in "Setup Dotfiles + Pakete" "Paketliste erstellen" "Hackdroid aktualisieren" "Beenden"; do
	case $REPLY in
		1) setup_dotfiles ;;
